...
void CONFIGURE_TIMER_FOR_RUN_TIME_STATS(void){
	...
	TIM_TimeBaseInitStruct.TIM_ClockDivision=TIM_CKD_DIV1;//时钟预分频
	TIM_TimeBaseInitStruct.TIM_CounterMode=TIM_CounterMode_Up;//选择计算模式
	TIM_TimeBaseInitStruct.TIM_Period=10-1;//Specifies the period value to be loaded into the activeAuto-Reload Register at the next update event.
	TIM_TimeBaseInitStruct.TIM_Prescaler=72-1;//pecifies the prescaler value used to divide the TIM clock.
	TIM_TimeBaseInitStruct.TIM_RepetitionCounter=0;//重复计算器（通用计算器无用）
