...
QueueHandle_t task_Count_Semaphore;
...
void start_task(void *pvParameters){
	...
	task_Count_Semaphore=xSemaphoreCreateCounting(100,50);
	if(task_Count_Semaphore!=NULL){
		printf("创建成功");
	}
...
void task1(void *pvParameters){
	...
	for(;;)
	{
		...
		if(keyNum==1){
			if(task_Count_Semaphore!=NULL)
			{
				err=xSemaphoreGive(task_Count_Semaphore);
				if(err==pdPASS)
				{
					printf("信号量释放成功\r\n");
				}else
...
void task2(void *pvParameters){
  ...
  for(;;)
	{
		err=xSemaphoreTake(task_Count_Semaphore,portMAX_DELAY);
		if(err==pdTRUE){
			printf("信号量计数值为：%d\r\n",(int)uxSemaphoreGetCount(task_Count_Semaphore));
		}
		vTaskDelay(1000);
	}
}
